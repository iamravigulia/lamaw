<div>
    ...
    <form action="{{route('fmt.lamaw.store')}}" method="post" class="fmt_box" enctype="multipart/form-data">
...
{{--  --}}
<form action="{{route('fmt.lamaw.csv')}}" method="POST" enctype='multipart/form-data' style="margin:20px 40px;">
    @csrf
    <input type="integer" name="problem_set_id" value="{{$pbs72 ?? ''}}" hidden style="border:1px solid #000000;">
    <input type="integer" name="format_type_id" value="{{$fmt72 ?? ''}}" hidden style="border:1px solid #000000;">
    ...
    <div style="display:block; padding:10px;">
        ...
        <input style="display:block;" type="file" name="audio_es[]" multiple accept="audio/*" placeholder="audio_es">
    </div>
